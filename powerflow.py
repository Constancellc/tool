...
nodes = {650:0,646:0,645:2,632:3,633:4,634:5,611:6,684:7,671:8,692:9,675:10,
         652:11,680:12}
...
A = matrix(0.0,(6*L+3*N,6*L+3*N))
b = matrix(0.0,(6*L+3*N,1))


# slack bus
for ph in range(0,3):
    A[9*L+ph,6*L+3*nodes[650]+ph] = 1.0
    b[9*L+ph] = 122 # V
...
for l in range(0,L):
    ...
    for ph in range(0,3):
        A[6*l+ph,6*l+ph] = 1.0 # p I think this bit is wrong
